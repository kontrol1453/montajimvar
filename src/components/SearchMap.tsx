  ...
  useEffect(() => {
    ...
    profiles.forEach((p) => {
      ...
      const icon = L.divIcon({
        html: `<div style="background:#ff7a00;color:white;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:bold;box-shadow:0 2px 8px rgba(0,0,0,0.3);border:3px solid white;cursor:pointer;">${p.companyName[0]?.toUpperCase() || "?"}</div>`,
        className: "",
        iconSize: [36, 36],
        iconAnchor: [18, 36],
      });
      ...
      const marker = L.marker(latlng, { icon })
        ...
        .bindPopup(`
          <div style="font-family:sans-serif;min-width:180px;">
            <a href="/firma/${p.id}" style="text-decoration:none;color:#ff7a00;font-weight:600;font-size:14px;">${p.companyName}</a>
            <div style="color:#666;font-size:12px;margin-top:4px;">${p.city} · ${p.categoryName}</div>
            ${stars ? `<div style="color:#ff7a00;font-size:13px;margin-top:2px;">${stars}</div>` : ""}
          </div>
        `);
