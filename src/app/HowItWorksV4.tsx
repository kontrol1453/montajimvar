        ...
        <div className="p-4 space-y-4">
          {[
            ...
            <div key={row.label}>
              ...
              <div className="h-9 rounded-lg bg-[var(--color-surface-secondary)] border border-[var(--color-border-light)] flex items-center px-3 text-sm text-[var(--color-dark)]">
                {row.value}
              </div>
            </div>
          ))}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-[var(--color-dark)]">
                  {offer.name}
                  {offer.selected && (
                    <span className="ml-2 text-[10px] text-[#00C853] font-medium">Seçildi</span>
                  )}
                </p>
                <p className="text-xs text-[var(--color-text-tertiary)]">
                  ⭐ {offer.rating} · {offer.days} gün
                </p>
              </div>
            </div>
          ...
          <div className="aspect-[4/3] rounded-lg bg-white/[0.03] relative overflow-hidden">
            ...
            <div className="absolute bottom-3 left-3 right-3 bg-white/[0.06] rounded-lg p-3 backdrop-blur-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-white text-xs font-medium">Ahmet Montaj Ekibi</p>
                  <p className="text-white/50 text-[10px]">Mobilya Montajı · Kadıköy</p>
                </div>
    ...
    visual: (
      <div className="bg-white rounded-xl border border-[var(--color-border-light)] overflow-hidden shadow-sm">
        <div className="p-4 border-b border-[var(--color-border-light)] flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-[var(--color-dark)]">Güvenli Ödeme</p>
            <p className="text-xs text-[var(--color-text-tertiary)]">Emanet sisteminde</p>
          </div>
        </div>
        <div className="p-4 space-y-3">
          <div className="flex items-center justify-between text-sm py-2 border-b border-[var(--color-border-light)]">
            <span className="text-[var(--color-text-tertiary)]">Toplam Tutar</span>
            <span className="font-bold text-[var(--color-dark)]">₺1.500</span>
          </div>
          <div className="flex items-center justify-between text-sm py-2 border-b border-[var(--color-border-light)]">
          ...
          <div className="flex items-center justify-between text-sm py-2">
            <span className="text-[var(--color-text-tertiary)]">İş Tamamlandı</span>
            <span className="text-[var(--color-text-tertiary)]">—</span>
          </div>
          <button className="w-full py-3 rounded-xl bg-[var(--color-surface-secondary)] text-sm font-semibold text-[var(--color-text-tertiary)] border border-[var(--color-border-light)] cursor-not-allowed mt-2">
            İşi Onayla (İş tamamlanınca)
          </button>
        </div>
      </div>
    ),
          ...
          <div className="relative">
            ...
            <div className="relative">
              <AnimatePresence mode="wait">
                ...
                >
                  {STEP_MOCKUPS[activeStep].visual}
                </motion.div>
              </AnimatePresence>
            </div>
          </div>
