...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const cleanDesc = profile.description
    ...
    : `${profile.companyName} - ${profile.city} ${profile.category.name} hizmeti. Montaj firması ile iletişime geçin, teklif alın.`;
  ...
  return {
    ...
    alternates: {
      canonical: `${baseUrl}/firma/${profile.id}`,
    },
    openGraph: {
      title: `${profile.companyName} - ${profile.city} ${profile.category.name}`,
      description: cleanDesc,
      type: "profile",
      locale: "tr_TR",
      siteName: "Montajım Var",
      url: `${baseUrl}/firma/${profile.id}`,
      images: [{ url: `${baseUrl}/firma/${profile.id}/opengraph-image` }],
    },
    twitter: {
...
export default async function CompanyProfilePage({ params }: Props) {
  ...
  const profile = await prisma.profile.findUnique({
    ...
    include: {
      ...
      user: { select: { id: true, name: true, email: true, phone: true, roles: true, createdAt: true, coverPhoto: true, bio: true } },
    },
  });
  ...
  const completedJobCount = await prisma.jobReview.count({
    where: {
      job: {
        offers: { some: { artisanId: profile.user.id, status: "accepted" } },
      },
    },
  });

  // Increment view count + log (skip owner's own views)
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Üst Bilgi */}
      <div className={`bg-dark-card rounded-xl border border-dark-border p-6 md:p-8 mb-6 ${profile.user.coverPhoto ? '-mt-20 relative z-10' : ''}`}>
        <div className="flex flex-col md:flex-row md:items-start gap-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-muted-text">
              {profile.ratingAvg > 0 && (
                <>
                  ...
                  <span className="text-sub-text">({profile.reviewCount} değerlendirme)</span>
                  <span>·</span>
                </>
              )}
              {completedJobCount > 0 && (
                <>
                  <span className="flex items-center gap-1">
                    ...
                    {completedJobCount} iş tamamlandı
                  </span>
                  <span>·</span>
                </>
              )}
            ...
            {profile.user.bio && (
              <p className="mt-3 text-sm text-sub-text italic">{profile.user.bio}</p>
            )}
      ...
      <ProfileSkills userId={profile.user.id} />

      {/* Video Portföy */}
      <ProfileVideos userId={profile.user.id} />

      {/* İş Değerlendirmeleri */}
      <div className="mb-6">
        <JobReviewsSection artisanId={profile.user.id} />
      </div>
